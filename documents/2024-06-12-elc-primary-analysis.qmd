...
\end{tabular}
Total Score: $< 1$ recommend ESC, 1-4 recommend Duloxetine, 7+ recommend ACT
\vfill

Can be made to include multiple subscales where features belong to one subscale (e.g. PROs) which are then combined, or all features can be included in a single mdoel.
\end{column}
\hfill
\begin{column}{0.5\textwidth}
Tree-based methods
...
Neither approach is universally superior, their relative performance depends on the underlying relationships.

Trees often perform comparatively well in settings with many categorical features and higher-order interactions, and worse in settings with strong, smooth parametric relationships.
...
We can investigate the expected improvement from receiving the recommended treatment versus receiving a random treatment.
```{r results='asis'}
