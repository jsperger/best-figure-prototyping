...
PlotSubgroupForest <- function(value_data, segments_data) {
  ...
  main_plot <- ggplot() +
    ...
    geom_point(
      data = value_data,
      aes(y = subgroup, x = value, shape = method),
      size = 3
    ) +
    scale_color_manual(
    ...
    labs(x = "Negative Change in 24-week PEG", y = 'Patient Characteristic') +
