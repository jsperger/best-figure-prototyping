...
PlotPredictorSummary <- function(in.data, num_predictors = 5) {
  ...
  # Create the plot
  pred_summary_plot <- ggplot(
    data_long,
    aes(y = Predictor, x = Value, group = interaction(Predictor, Metric))
  ) +
    geom_segment(
      aes(yend = as.numeric(Predictor) + Offset, xend = 0),
      color = "grey50",
      size = 0.5
    ) + # Draw the lollipop sticks with offset
    geom_point(
      aes(color = ifelse(Metric == "Magnitude", Sign, NA)),
      size = 3,
    ...
    scale_color_manual(
      ...
      na.value = "black"
    ) + # Define colors, default black for uniqueness
...
PlotPredImportance <- function(in.data, num_predictors = 5) {
  ...
  pred_summary_plot <- ggplot(
    in.data,
    aes(y = Predictor, x = Magnitude, group = interaction(Predictor, Magnitude))
  ) +
    geom_segment(
      aes(xend = 0, yend = Predictor, color = Sign, linetype = Sign),
      size = .5
    ) +
...
PlotPredPubr <- function(in.data, num_predictors = 5) {
  ...
  pred_summary_plot <- ggdotchart(
    ...
    labs(
      ...
      label = "Effect on Treatment Efficacy"
    ) + # Label axes
    theme(
      axis.text.y = element_text(size = 8, angle = 30, hjust = 1),
...
.PlotPredSlice <- function(in.data, num_predictors, grouping_var = NULL) {
  ...
  if (!is.null(grouping_var)) {
    top_pred_data <- top_pred_data %>%
      ...
      mutate(
        Predictor = factor(
          paste(Predictor, ":", !!sym(grouping_var)),
...
PlotPredictorLollipopGrouped <- function(
  in.data,
  num_predictors = 5,
  grouping_var = NULL
) {
