...
#'
FakeDataForPredictorSummary <- function(
  treatments = c("ESC", "EBEM", "ACT", "Duloxetine")
) {
  # Create a data frame
  ...
  test_data$Uniqueness <- runif(n = nrow(test_data), min = 0, max = 1) *
    test_data$Magnitude
...
FakeDataForValueComparison <- function(
  policy_names,
  point_estimates = NULL,
  ci_half_widths = NULL
) {
  provided_est_flag <- all(is.null(point_estimates), is.null(ci_half_widths)) ==
    FALSE

  if (provided_est_flag == TRUE) {
    if (!is.null(point_estimates) && is.null(ci_half_widths)) {
      stop(
        "If point estimates are provided, CI half-widths must also be provided."
      )
    }

    if (
      length(point_estimates) != length(policy_names) ||
        length(ci_half_widths) != length(policy_names)
    ) {
      stop(
        "Length of point estimates and CI half-widths must match the length of policy names."
      )
    }
  }
...
#
FakeDataForDTRAssignments <- function(
  n_participants = 800,
  ...
  tibble(
    ...
    mutate(
      stage2_action_allocation = case_when(
        stage1_allocation == "ACT" ~
          sample(
            stage2_actions,
            size = 1,
            replace = TRUE,
      ...
      stage2_treatment_allocation = case_when(
        ...
        stage2_action_allocation %in% c('Augment', "Switch") ~
          sample(
            stage2_treatments[which(stage2_treatments != stage1_allocation)],
            size = 1
          )
      ),
...
FabricateTypedData <- function(n.subj.per.trt) {
  ...
  fake_data <- fabricatr::fabricate(
    ...
    PainSeverity = draw_ordered(
      N = N,
      x = runif(N),
      outcome_labels = c("None", "Mild", "Moderate", "Severe")
    ), # Pain Severity as an ordered factor
    ...
    FunctionalStatus = draw_categorical(
      N = N,
      prob = c(0.3, 0.4, 0.3),
      outcome_labels = c("Low", "Medium", "High")
    ), # Functional Status as a categorical variable
    EmploymentStatus = draw_categorical(
      N = N,
      prob = c(0.6, 0.2, 0.2),
      outcome_labels = c("Employed", "Unemployed", "Retired")
    ...
    PreviousSurgery = draw_binary(
      N = N,
      prob = 0.4,
      outcome_labels = c("No", "Yes")
    ), # Previous Surgery as a binary variable
    SmokingStatus = draw_categorical(
      N = N,
      prob = c(0.7, 0.2, 0.1),
      outcome_labels = c("Never", "Former", "Current")
    ), # Smoking Status as a categorical variable
    AlcoholUse = draw_categorical(
      N = N,
      prob = c(0.5, 0.3, 0.2),
      outcome_labels = c("None", "Moderate", "Heavy")
    ), # Alcohol Use as a categorical variable
    ExerciseFrequency = draw_ordered(
      N = N,
      x = runif(N),
      outcome_labels = c("Never", "Rarely", "Sometimes", "Often", "Always")
    ), # Exercise Frequency as an ordered factor
    ...
    SleepQuality = draw_ordered(
      N = N,
      x = runif(N),
      outcome_labels = c("Very Poor", "Poor", "Fair", "Good", "Very Good")
    ), # Sleep Quality as an ordered factor
    DietQuality = draw_ordered(
      N = N,
      x = runif(N),
      outcome_labels = c("Very Poor", "Poor", "Fair", "Good", "Very Good")
    ), # Diet Quality as an ordered factor
    SocialSupport = draw_categorical(
      N = N,
      prob = c(0.3, 0.4, 0.3),
      outcome_labels = c("Low", "Medium", "High")
    ), # Social Support as a categorical variable
    PastTherapyAdherence = draw_categorical(
      N = N,
      prob = c(0.2, 0.3, 0.5),
      outcome_labels = c("None", "Partial", "Full")
    ), # Past Therapy Adherence as a categorical variable
    PainCopingStrategies = draw_ordered(
      N = N,
      x = runif(N),
      outcome_labels = c("None", "Some", "Moderate", "Good", "Excellent")
    ), # Pain Coping Strategies as an ordered factor
...
FakeDataDescriptiveAnalyses <- function(n.subj = 800) {
  ...
  wide_data <- tibble(
    ...
    PGIC = unlist(lapply(treat_assignments, function(t) {
      sample(1:7, 1, prob = pgic_weights[[t]], replace = TRUE)
    }))
  ) %>%
...
SummarizeDescriptiveOutcomes <- function(
  results_tbl_df,
