...
CreateExDTRRespDifPlot <- function() {
  ...
  pmed_plot <- ggplot(
    ...
    labs(
      subtitle = "Why both treatment recommendations and response predictions are needed",
      x = "Age",
...
PlotValueComparison <- function(in.data) {
  ...
  p <- ggplot(
    in.data,
    aes(x = PointEstimate, y = TreatmentPolicy, color = TreatmentPolicy)
  ) +
    ...
    theme(
      axis.text.y = element_text(size = 10), # Increase font size for y axis texts
      ...
      plot.margin = margin(t = 3.5, r = 0.5, b = 2.5, l = 0.5, unit = "cm")
    ) # Adjust plot margins)
